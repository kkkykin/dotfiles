...
cd %USERPROFILE%
start /b runemacs.exe --daemon "--init-directory=%initd%" -l "%profileel%"
#+end_src
...
** trojan
:PROPERTIES:
:header-args: :var trojan-dir=(substitute-in-file-name "$USERPROFILE/scoop/app/trojan-go")
:END:
...
start /b /abovenormal /d "%USERPROFILE%\scoop\app\trojan-go" trojan-go.exe
#+end_src
