...
(let* ((zr-local-pls (plstore-open "ssh.pls"))
       ...
       (sshd-port (plist-get (cdr (plstore-get zr-local-pls "sshd")) :port))
       (zr-local-pls (zr-sops-parse-json "../global.json"))
       (main-user (gethash "main" (gethash "users" zr-local-pls)))
...
#+name: over-tls
#+begin_src conf-space
ProxyCommand openssl s_client -quiet -connect example.com:443 -servername example.com
#+end_src

* sshd
:PROPERTIES:
:CUSTOM_ID: 24511934-a6f2-41a1-a951-9063eb8218b2
:header-args:sh: :shebang "#/usr/bin/env bash" :var user=(identity main-user)
:END:

#+begin_src sh :tangle (zr-org-by-tangle-dir "user-auth.sh")
adduser "$user"
apt install sudo
...
#+header: :var sshd-port=(identity sshd-port)
