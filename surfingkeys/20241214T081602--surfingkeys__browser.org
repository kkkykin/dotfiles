...
#+begin_src js
function callBrowserScriptBridge(detail) {
    detail.mask = browser_script_bridge_mask;
    window.dispatchEvent(new CustomEvent('browser-script-bridge', { detail }));
}

mapkey(',l', 'org store link', () => {
    callBrowserScriptBridge({
        action: 'store-link',
        url: location.href,
...
mapkey(',c', 'org capture', () => {
    callBrowserScriptBridge({
        action: 'capture',
        url: location.href,
        title: document.title,
        body: getSelection().toString()
    });
})
#+end_src
...
const zrProxyConf = <<sops-json-to-js-obj(file="../pac.json")>>
#+end_src

#+begin_src js
const updateProxy = (mode) => {
  const { proxy, autoproxy_hosts } = zrProxyConf;
  const defaultProxy = proxy.at(-1);

  const [start, end] = mode === 'bypass' 
    ? [0, 1] 
    : [1, autoproxy_hosts.length];

  const hosts = autoproxy_hosts.slice(start, end);
  const proxies = hosts.map(() => defaultProxy);

  RUNTIME('updateProxy', { operation: 'set', host: hosts, proxy: proxies, mode });
};
...
var pacGlobal = <<sops-json-to-js-obj(file="../pac.json")>>
...
function loadScript(url, onload, name) {
  fetchUrl(url, function(scriptText) {
    try {
      ...
      Front.showBanner(`${name} 执行失败`);
      console.error(e);
    }
  }, name);
}

function loadScriptWithExports(url, exports, onload, name) {
  fetchUrl(url, function(scriptText) {
    try {
...
***** fswp
:PROPERTIES:
:CUSTOM_ID: 7ae2822d-7721-4b42-98da-e9592326ad9f
:END:

#+begin_src js :tangle no
loadScriptWithExports(
  'https://github.com/redguardtoo/fswp/raw/refs/heads/main/js/fswp.js',
  ['fswpToggleSearchInterface'],
  function(fswp) {
    mapkey(',/', '#Fuzzy search webpage', function() {
      fswp.fswpToggleSearchInterface();
    });
  },
  'fswp.js'
);
