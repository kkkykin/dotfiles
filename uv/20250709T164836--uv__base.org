...
def choose_url(image: PixivImage, preferred_size: str) -> str:
    ...
    if image.urls:
        return next(iter(image.urls.values()))
...
def termux_set_wallpaper(image_path: Path, *, lockscreen: bool, both: bool) -> None:
    ...
    if both:
        subprocess.run(["termux-wallpaper", "-f", str(image_path)], check=True)
        subprocess.run(["termux-wallpaper", "-l", "-f", str(image_path)], check=True)
        return
