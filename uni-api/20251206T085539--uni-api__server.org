...
(let* ((zr-local-pls (plstore-open "../.global.pls"))
       (zr-org-tangle-default-dir
        (if zr-sys-linux-p
            (plist-get (cdr (plstore-get zr-local-pls "private-www")) :dir)
          "_tangle"))
...
#+header: :var iflow=iflow[]
#+header: :var modelscope=modelscope[]
#+name: gen-config
#+begin_src elisp
           ...
           (lambda (models)
             (cl-map
              ...
              (lambda (m)
                (if-let* ((model (car m))
                          (alias (cadr m))
                          ((string-empty-p alias)))
                    model
                  (list (cons (intern model) alias))))
              models))))
