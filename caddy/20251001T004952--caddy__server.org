...
(let* ((name (pcase system-type
       ...
       (siteproxy (gethash "siteproxy" zr-local-pls))
       (zr-local-pls (zr-sops-parse-json "caddy.json"))
       (caddy-proxy (gethash "proxy" zr-local-pls))
...
#+header: :var siteproxy=(concat siteproxy "/")
...
:END:
#+header: :var siteproxy="{$SITEPROXY}"
...
def uri_options:
  ...
  | [
      ...
      {
        pattern: "^https://[^/]+[.]imoutoheaven[.]org/d/",
        options: {"max-connection-per-server": "2"}
      },
      {
        pattern: "^https://catcat[.]cloud/d/",
        options: {"max-connection-per-server": "2"}
      }
    ]
  | reduce .[] as $rule ({};
...
(mapconcat (lambda (s) (format "%s 1" s)) (gethash cors zr-local-pls) "\n")
#+end_src
