...
(let* ((name (pcase system-type
       ...
       (caddy-proxy (gethash "caddy-proxy" zr-local-pls))
...
******* trans-forward
:PROPERTIES:
:CUSTOM_ID: 5e925cc0-b6ca-4e5c-b812-184f6fc32c99
:END:
#+begin_src caddy :tangle (zr-org-by-tangle-dir "trans-forward.Caddyfile")
(trans-forward) {
    ...
    handle @http {
        ...
        reverse_proxy {http.regexp.up.1}:{up_port} {
            ...
            header_up Host {http.regexp.up.1}
...
handle_path /rsshub/* {
    reverse_proxy <<rsshub-upstreams()>> https://rss.sakiko.de https://kayin.zeabur.app {
...
handle_path /aria2-jsonrpc {
    ...
    json_parse @json {
      ...
      merge_if_match params.0 ^https://[^/]+\.imoutoheaven\.org/d/ params.1 {"max-connection-per-server":"2"} when size({json.params}) == 2
      merge_if_match params.1 ^https://[^/]+\.imoutoheaven\.org/d/ params.2 {"max-connection-per-server":"2"} when size({json.params}) == 3
      merge_if_match params.0 ^https://catcat\.cloud/d/ params.1 {"max-connection-per-server":"2"} when size({json.params}) == 2
      merge_if_match params.1 ^https://catcat\.cloud/d/ params.2 {"max-connection-per-server":"2"} when size({json.params}) == 3
    }
...
#+header: :var proxy-auth-header=(gethash "header" caddy-proxy)
#+header: :var proxy-auth-key=(gethash "key" caddy-proxy)
      ...
      (dolist (tbody tbodies)
        (dolist (tr (dom-by-tag tbody 'tr))
          ...
          (when-let* ((first-td (car (dom-by-tag tr 'td)))
                      ...
                      ((not (member href '("https://rsshub.app"
