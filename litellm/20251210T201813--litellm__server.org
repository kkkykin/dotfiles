         ...
         (litellm-settings '((drop_params . t)
                             ...
                             (custom_provider_map
                              . [((provider . "ms")
                                 ...
                                 ((provider . "poll")
                                  (custom_handler . "pollinations_handler.pollinations_llm"))])
...
* models
#+name: models-tbl
| name                          | poll   | nvidia                              | cpa   | modelscope                          | 88code |
|-------------------------------+--------+-------------------------------------+-------+-------------------------------------+--------|
...
#+begin_src py
"""
Base OpenAI Delegate Handler for LiteLLM Custom Providers

Provides shared Chat/Completion logic that delegates to LiteLLM's OpenAI-compatible handler.
Subclasses only need to implement provider-specific features (e.g., image generation).
"""
...
def _sanitize_params_for_delegation(optional_params: Optional[dict], extra_keys_to_remove: list = None) -> dict:
...
Chat/Completion: delegates to LiteLLM's OpenAI-compatible handler (via base class)
...
class ModelScopeLLM(OpenAIDelegateHandler):
    ...
    def image_generation(
        ...
        model_response.data = [
            ImageObject(url=img if isinstance(img, str) else img.get("url"), b64_json=None)
    ...
    async def aimage_generation(
        ...
        output_images = result.get("output_images", []) or []
        image_urls = [img if isinstance(img, str) else img.get("url") for img in output_images]
...
** pollinations
:PROPERTIES:
:CUSTOM_ID: 9ad028c5-db07-4cc5-aa06-b403ab6472a2
:END:
#+begin_src py :tangle (zr-org-by-tangle-dir "pollinations_handler.py")
"""
Pollinations Custom LLM Handler for LiteLLM

Chat/Completion: delegates to LiteLLM's OpenAI-compatible handler (via base class)
Image Generation: GET request to Pollinations API with direct binary response

Usage in config.yaml:
    model_list:
      - model_name: "pollinations-chat"
        litellm_params:
          model: "pk/openai/gpt-4"
    ...
    litellm_settings:
      custom_provider_map:
        - {"provider": "pk", "custom_handler": pollinations_handler.pollinations_llm}
"""
...
POLLINATIONS_REQUEST_TIMEOUT = 120  # 2 minutes for image generation
...
class PollinationsLLM(OpenAIDelegateHandler):
    ...
    def _build_image_url(
        ...
        """
        Build Pollinations image generation URL.
        ...
        if isinstance(size, str) and "x" in size:
            ...
            params.append(f"width={quote(str(width), safe='')}")
            params.append(f"height={quote(str(height), safe='')}")

        if "seed" in optional_params and optional_params["seed"] is not None:
            params.append(f"seed={quote(str(optional_params['seed']), safe='')}")

        if "quality" in optional_params and optional_params["quality"] is not None:
            params.append(f"quality={quote(str(optional_params['quality']), safe='')}")
        ...
        if "guidance_scale" in optional_params and optional_params["guidance_scale"] is not None:
            params.append(f"guidance_scale={quote(str(optional_params['guidance_scale']), safe='')}")
        ...
        if "negative_prompt" in optional_params and optional_params["negative_prompt"]:
            params.append(f"negative_prompt={quote(str(optional_params['negative_prompt']), safe='')}")
        ...
        if "image" in optional_params and optional_params["image"]:
            ...
            if isinstance(image_param, list):
                image_param = "|".join(image_param)
            params.append(f"image={quote(str(image_param), safe='')}")
    ...
    def image_generation(
        ...
        Makes a GET request to /image/{prompt} and returns the binary image as Base64.
        """
        optional_params = dict(optional_params or {})
        ...
        model_response.data = [ImageObject(url=None, b64_json=b64_data)]
        return model_response
