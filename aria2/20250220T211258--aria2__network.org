...
(dolist (l '("https://github.com/P3TERX/aria2.conf/raw/refs/heads/master/dht.dat"
  ...
  (let* ((url (zr-proxy-transform-url l))
         (name (expand-file-name (file-name-nondirectory l) "_tangle"))
         (etag (file-name-with-extension name "txt")))
    (call-process "curl" nil 0 nil "-Lo" name
...
(let ((zr-local-pls (plstore-open "aria2.pls"))
  ...
  (org-babel-tangle nil nil "^conf-unix$")
...
(let ((nix-dir (expand-file-name "~/nix-config")))
  (when (and (eq 'gnu/linux system-type)
             (file-directory-p nix-dir)
             (executable-find "sops"))
    (org-babel-tangle nil nil "^nix$")))
#+end_src
...
#+begin_src conf-unix :var dir=(if (eq 'gnu/linux system-type) (expand-file-name "downloads" "/var/lib/aria2") "./downloads") alloc=(if (y-or-n-p "SSD? ") "none" "falloc")
...
** module
:PROPERTIES:
:CUSTOM_ID: 109e43d4-f990-4e01-a20c-916a3c6171d2
:END:
#+begin_src nix :tangle (expand-file-name "modules/nixos/aria2.nix" nix-dir)
{
  config,
  username,
  ...
...
#+end_src
** settings
#+name: gen-nix-settings
#+begin_src elisp
(let* ((secret-file (expand-file-name "~/nix-config/secrets/aria2.json"))
       (sops (executable-find "sops"))
       (key "[\"aria2_rpc_secret\"]")
       (secret (and (file-exists-p secret-file)
                    (with-temp-buffer
                      (call-process sops nil t nil
                                    "decrypt" "--extract" key secret-file)
                      (buffer-string)))))
  (with-temp-buffer
    (insert-file-contents "_tangle/aria2.conf")
    (goto-char (point-min))
    (while (zerop (forward-line))
      (let* ((bol (pos-bol))
             (eol (pos-eol))
             (line (buffer-substring bol eol))
             (secret-flag "rpc-secret=")
             (regexp (rx bol (+ (not ?=)) ?= (| "true" "false" (+ (| ?. digit))) (* ? ) eol)))
        (cond
         ((looking-at-p "#") (delete-region bol eol))
         ((string-match-p regexp line) (end-of-line) (insert ";"))
         (t
          (delete-region bol eol)
          (if (string-prefix-p secret-flag line)
              (when-let* ((new-secret (substring line (length secret-flag)))
                          ((not (equal new-secret secret))))
                (print secret)
                (print new-secret)
                (call-process sops nil 0 nil "set" secret-file
                              key (json-serialize new-secret)))
            (insert (replace-regexp-in-string
                     (rx bol (group (* (not ?=))) ?= (group (* any)) eol)
                     "\\1=\"\\2\";" line)))))))
    (goto-char (point-min))
    (flush-lines "^ *$")
    (buffer-string)))
...
def update_fdroid_repo(gid, file):
  """更新 fdroid repo"""
  if file.endswith('.apk'):
    from update_fdroid_repo import update_fdroid_repo
    return update_fdroid_repo(file)
  

def event_handler(gid, count, file=None):
  """Handle aria2 events and trigger hooks."""
  if int(count) > 1:
    for func in on_bt_download_complete_functions:
      if func(gid, count):
        break
  else:
    for func in on_download_complete_functions:
      if func(gid, file):
        break

# Register completion hooks
on_download_complete_functions.append(update_fdroid_repo)
...
*** update_fdroid_repo
:PROPERTIES:
:CUSTOM_ID: 394e05c8-f9e4-4a99-a223-bba878f0b471
:END:
#+begin_src python :tangle (zr-org-by-tangle-dir "update_fdroid_repo.py")
import os
import shutil
import subprocess
from datetime import datetime

fdroid_path = '/mnt/mediadata/fdroid/'

def is_fdroid_path_writable():
  """检查 fdroid_path 是否可写"""
  return os.access(fdroid_path, os.W_OK)
...
def move_apk_file(file_path):
  """移动 APK 文件到 repo 目录"""
  try:
    base_name = os.path.basename(file_path)
    new_filename = generate_timestamped_filename(base_name)
    
    repo_path = os.path.join(fdroid_path, 'repo')
...
def execute_fdroid_commands(log_file):
  """执行 fdroid 命令并记录日志"""
  try:
    with open(log_file, 'a') as log:
      ...
      # 执行 fdroid update
      result1 = subprocess.run(['fdroid', 'update'], 
                            cwd=fdroid_path, 
                            capture_output=True, 
                            text=True)
      log.write("fdroid update output:\n")
      log.write(result1.stdout)
      if result1.stderr:
        log.write("fdroid update errors:\n")
        log.write(result1.stderr)
      
      # 执行 fdroid deploy -v
      result2 = subprocess.run(['fdroid', 'deploy', '-v'], 
                            cwd=fdroid_path, 
                            capture_output=True, 
                            text=True)
      log.write("fdroid deploy -v output:\n")
      log.write(result2.stdout)
      if result2.stderr:
        log.write("fdroid deploy -v errors:\n")
        log.write(result2.stderr)
    
    print("fdroid commands executed successfully")
    return True
    
  except Exception as e:
    print(f"Error executing fdroid commands: {str(e)}")
    return False


def update_fdroid_repo(file):
  """更新 fdroid 仓库"""
  # 检查 fdroid_path 是否可写
  if not is_fdroid_path_writable():
    print(f"Error: {fdroid_path} is not writable")
    return False
  
  try:
    # 移动 APK 文件
    flag = move_apk_file(file)
    
    # 执行 fdroid 命令并记录日志
    log_file = os.path.join(fdroid_path, 'fdroid_update.log')
    execute_fdroid_commands(log_file)
    return flag
