...
#+begin_src lua
local function term_chdir(path)
  if path and vim.fn.isdirectory(path) then
    vim.api.nvim_buf_set_var(0, 'osc7_dir', path)
  ...
  ['cmd.exe'] =
   {
     ['<C-w>n'] = function()
       vim.cmd.stopinsert()
       local proc = term_last_matching_proc()
       if proc then
         term_chdir(proc.match[2])
       end
     end,
     ['<C-p>'] = '<Up>',
     ['<C-n>'] = '<Down>',
...
local function setup_terminal()
  ...
  if bufname:match('^term://') then
    ...
    vim.api.nvim_buf_set_var(0, 'term_key_cache', {
      ['proc'] = cmd,
      ['line_num'] = 1,
    })
    for key, _ in pairs(term_keymaps[cmd]) do
