...
:END:
#+begin_src lua
-- https://github.com/wezterm/wezterm/issues/5007#issuecomment-2481674933
...
:END:
#+begin_src lua
local function escape_elisp_string(s)
  return (s:gsub("\\", "\\\\"):gsub('"', '\\"'))
end

local function emacs_eval(sexp, cb)
  cb = cb or function(_) end

  vim.system({ "emacsclient", "-n", "-e", sexp },
             { text = true },
             function(res)
...
***** dictionary
:PROPERTIES:
:CUSTOM_ID: 00830061-38d8-4bac-9d60-8a7acf7d17c3
:END:
#+begin_src lua
local function dictionary_search_under_cursor()
  ...
  local sexp = string.format('(dictionary-search "%s")', escape_elisp_string(word))
  emacs_eval(sexp, function(res)
                     if res.code ~= 0 then
                       local msg = (res.stderr and res.stderr ~= "" and res.stderr)
                         or ("emacsclient exited with code " .. tostring(res.code))
...
vim.keymap.set("n", "<leader>d", dictionary_search_under_cursor, { desc = "Emacs dictionary-search under cursor" })
#+end_src
...
if (vim.uv or vim.loop).fs_stat(lazypath) then
  ...
  local already_in_rtp = vim.tbl_contains(rtp, lazypath)

  -- 只在 lazypath 还不在 rtp 时，做一次性初始化
