...
runMacro(keyName) {
  ...
  for dirName in dirNames {
    fullPath := gMacroDir '\' dirName '\' keyName '.ahk'
...
SaveMacroInternal(pMacro, pDir, pKey){
  ...
  descGui := Gui()
  descGui.Title := '输入宏描述'
  descGui.Add('Text',, '请输入宏描述:')
  descEdit := descGui.Add('Edit', 'w200 vdesc')
  descGui.Add('Button', 'default', 'OK').OnEvent('Click', SaveWithDescription)
  descGui.OnEvent('Close', (*) => descGui.Destroy())
...
SaveMacro() {
  static sLastRecordedMacroFile := A_Temp '/~Record1.ahk'
  if not FileExist(sLastRecordedMacroFile){
  ...
  ProcessUserInput(*) {
    ...
    if selectedRow {
      ...
      dir := gMacroDir '/' Saved.dir
      SaveMacroInternal sLastRecordedMacroFile, dir, key
    } else {
      MsgBox '请先选择一个宏键'
    }
  }
...
macroManager(){
  ...
  if processLiveP(macroRunningPid, 'AutoHotkeyUX.exe') {
    ...
    Run '"' A_AhkPath '" /restart "' gOrgDir 'lib/AHK-Macro-Recorder/Macro Recorder.ahk"',,, &macroRecorderPid
