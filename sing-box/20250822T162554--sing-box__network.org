...
#+header: :var key_log_file=(expand-file-name "key.log" "_tangle/naive/")
#+begin_src json :tangle (expand-file-name "config.json" "_tangle/naive") :mkdirp t
{
  "listen": "socks://127.0.0.1:10807",
  "proxy": "https://$nv_user_name:$nv_user_pass@$nv_host",
  "host-resolver-rules": "MAP $nv_host $nv_ip",
  "ssl-key-log-file": "$key_log_file"
}
#+end_src
...
(let* ((raw (multisession-value zr-net-proxy-rules-hash))
  ...
  (json-serialize
   `((version . 1)
     (rules . ,(vconcat (append local `((domain_suffix ,(vconcat suffix)))))))))
#+end_src
...
(let* ((default '((domain_suffix [])))
  ...
  (json-serialize
   `((version . 1)
     (rules . ,(vconcat (append local default))))))
#+end_src
