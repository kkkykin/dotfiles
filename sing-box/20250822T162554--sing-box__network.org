          ...
          (mapc
           (lambda (obd)
             (when-let* (((not (member (alist-get "type" obd nil nil #'string=)
               ...
               (when (string-match-p "中继" tag)
                 (push tag relay-outbounds))
               (push tag sub-outbounds)
               (push (push `(tag . ,tag) obd) obds)))
           outbounds)
          (if sub-outbounds
              (let* ((extra `(((type . "selector")
                               (tag . ,selector)
                               (outbounds . ,(vconcat (cons urltest sub-outbounds)))
                               (default . ,urltest)
                               (interrupt_exist_connections . :false))
                              ((type . "urltest")
                               (tag . ,urltest)
                               (outbounds . ,(vconcat sub-outbounds)))))
...
#+name: gen-this-way
#+begin_src elisp
(let* ((s-this-way '("u[this-way]" "s[self]" "s[sub]"))
       (u-this-way (car s-this-way))
       (u-relay "u[relay]")
       (s-relay (cons u-relay relay-outbounds))
       (u-sub "u[sub]")
       (s-sub (cons u-sub sub-outbounds))
       (pre-obds `(((type . "selector")
                    (tag . "s[this-way]")
                    (outbounds . ,(vconcat s-this-way))
                    (default . ,u-this-way))
                   ((type . "urltest")
                    (tag . ,u-this-way)
                    (outbounds . ,(vconcat (cdr s-this-way))))
                   ((type . "selector")
                    (tag . "s[sub]")
       ...
       (relay-obds (and relay-outbounds
                        `(((type . "selector")
                           (tag . "s[relay]")
                           (outbounds . ,(vconcat s-relay))
                           (default . ,u-relay))
                          ((type . "urltest")
                           (tag . ,u-relay)
                           (outbounds . ,(vconcat (cdr s-relay)))))))
       (coding-system-for-read 'utf-8))
  (json-serialize `((outbounds . ,(vconcat pre-obds relay-obds)))))
#+end_src
