...
(let* ((zr-local-pls (plstore-open (expand-file-name "sing-box.pls" zr-secrets-dir)))
...
#+begin_src json :tangle (zr-org-by-tangle-dir "500-main.json") :mkdirp t
{
  ...
  "route": {
    ...
    "rule_set": [
      ...
      {
        "path": "$rule_sets_dir/server-dns-redirect.srs",
        "tag": "server-dns-redirect-rules",
        "type": "local"
      },
      {
        "path": "$rule_sets_dir/server-direct.srs",
...
<<gen-server-bounds(name="vps0",token-key="cf0")>>
#+end_src
...
<<gen-server-bounds(name="vps1",token-key="cf1")>>
#+end_src
...
<<gen-server-bounds(name="vps2",token-key="cf2")>>
#+end_src
...
* bounds
:PROPERTIES:
:header-args:elisp: :var wg-clients=(cdr (plstore-get zr-local-pls "wg")) ts-ep='((tag . "ts-ep") (type . "tailscale")) obds='(((tag . "direct") (type . "direct"))) ibds='() ep='() conf='() dns-rules='()
:END:
** client
...
(let ((tailscale-state-dir (expand-file-name "tailscale_state" "_tangle"))
  ...
  (dolist (info vps)
    (let* ((name (car info))
      ...
      (when-let* ((wg (plist-get wg-clients (intern (concat ":" (system-name)))))
                  (port (plist-get attr :wg-port))
                  (addr (plist-get attr :wg-addr))
                  (pub (plist-get attr :wg-pub-key))
                  (proto (if (and (eq ip-type :ipv6)
                                  (not zr-sys-android-p))
                             :ipv6
                           :ip))
                  (peers `( :address ,(plist-get attr proto)
                            :port ,port
        ...
        (push `( (tag . ,tag)
                 ...
                 (address . ,(concat (plist-get wg :addr) "/24"))
                 (private_key . ,(plist-get wg :priv))
                 (peers . ,(vector peers)))
              ep))))
...
(let* ((info (plstore-get zr-local-pls name))
       ...
       (user-pass (plist-get attr :user-pass))
       dns-answer
       (cf-token (plist-get (cdr (plstore-get zr-local-pls token-key)) :api-token)))

  (when-let* ((ip (plist-get attr :ip)))
    (push (format "localhost. IN A %s" ip) dns-answer)
    (push `((listen . ,ip)
            (listen_port . 53)
            (network . "udp")
            (tag . "dns4-in")
            (type . "direct"))
          ibds)
    (when-let* ((ipv6 (plist-get attr :ipv6)))
      (push (format "localhost. IN AAAA %s" ipv6) dns-answer)
      (push `((listen . ,ipv6)
              (listen_port . 53)
              (network . "udp")
              (tag . "dns6-in")
              (type . "direct"))
            ibds))
    (push `((action . "predefined")
            (answer . ,(vconcat dns-answer))
            (mode . "and")
            (rcode . "NOERROR")
            (rules . [((rule_set . "geoip-cn")
                       (rule_set_ip_cidr_match_source . t))
                      ((rule_set . "server-dns-redirect-rules"))
                      ((protocol . "dns"))])
            (type . "logical"))
          dns-rules))

  (when-let* ((port (plist-get attr :wg-port))
              (addr (plist-get attr :wg-addr))
              (priv-key (plist-get attr :wg-priv-key))
              (peers (cl-loop for p in wg-clients
                              for i from 0
                              when (cl-oddp i)
                              collect
                              `( :public_key ,(plist-get p :pub)
                                 :allowed_ips ,(concat (plist-get p :addr) "/32")))))
    (push `((tag . "wg-ep")
            (type . "wireguard")
            (system . t)
            (address . ,addr)
...
(let* ((local-file "_browse.eld")
  ...
  (json-serialize
   `((version . 3)
     (rules . ,(vconcat '(((process_path_regex
                            . ["[\\\\/]chrome(\\.exe)?$"
                               "[\\\\/]cromite(\\.exe)?$"
                               "[\\\\/]cli-proxy-api(\\.exe)?$"
                               "[\\\\/]firefox(\\.exe)?$"
...
** server-dns-redirect
:PROPERTIES:
:CUSTOM_ID: 87356417-1776-4026-8df3-18f1895c6f23
:END:
#+begin_src json :mkdirp t :tangle (zr-org-by-tangle-dir "server-dns-redirect.json")
{
  "rules": [
    {
      "domain": [
