...
(let* ((zr-local-pls (plstore-open "sing-box.pls"))
  ...
  (when-let* (((eq 'gnu/linux system-type))
              (sops (executable-find "sops"))
              (secrets-dir (expand-file-name "~/nix-config/secrets"))
              (file-directory-p secrets-dir)
              (sops-enc-file (expand-file-name "sing-box.json" secrets-dir)))
    (call-process sops nil (list :file sops-enc-file) nil
                  "--encrypt" tun-file))
