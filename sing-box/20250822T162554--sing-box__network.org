...
#+name: gen-server-dns
#+begin_src elisp
(let* ((cf-dns-tag "cf-dot")
       (hosts-tag "local-hosts")
       (mydomain (auth-source-pick-first-password :host "mydomain" :user "main"))
       (server-hosts-map (gethash "server-hosts-map" box-secrets))
       (dns-servers `(((accept_default_resolvers . :false)
                       (endpoint . "s[tailscale]")
                       (tag . "tailscale-dns")
                       (type . "tailscale"))
                      ((server . "1.1.1.1")
                      ...
                      ((type . "hosts")
                       (tag . ,hosts-tag)
                       (predefined . ,server-hosts-map))))
       (dns-rules `(((domain . ,(vconcat (hash-table-keys server-hosts-map)))
                     (server . ,hosts-tag))
                    ...
                    ((network . "tcp")
                     ...
                     (client_subnet . "114.114.114.114/24")))))
  (json-serialize
   `((dns . ((final . ,cf-dns-tag)
             (strategy . "prefer_ipv6")
             (servers . ,(vconcat dns-servers))
             (rules . ,(vconcat dns-rules))))
     (route . ((default_domain_resolver . ,cf-dns-tag)
               (rules
                . [((action . "sniff")
                    (sniffer . "dns"))
                   ((action . "hijack-dns")
...
#+end_src
* dns
#+begin_src python :shebang "#!/usr/bin/env -S uv run -qw dnspython"
# /// script
# dependencies = [
#   "dnspython",
# ]
