...
(let* ((box-secrets (with-temp-buffer
       ...
       (extra-dns-servers `(((accept_default_resolvers . :false)
                             ...
                             (tag . "tailscale-dns")
                             (type . "tailscale"))))
       (pre-dns-rules `(((domain_suffix . ,(concat "." mydomain))
                         (action . "predefined")
...
(let* ((gg-dns-tag "gg-dns")
       ...
       (dns-rules `(((network . "tcp")
                     ...
                     (client_subnet . "114.114.114.114/24")))))
  (json-serialize
   `((dns . ((final . ,gg-dns-tag)
             (strategy . "prefer_ipv6")
             (servers . ,(vconcat extra-dns-servers dns-servers))
             (rules . ,(vconcat pre-dns-rules dns-rules))))
     (route . ((default_domain_resolver . ,gg-dns-tag)
               (rules
                . [((action . "sniff")
                    (sniffer . "dns"))
                   ((action . "hijack-dns")
...
#+header: :var cf_domain="youxuan.cf.090227.xyz"
#+begin_src python
import argparse
import sys
import socket
