...
#+header: :python "uv run -qw dnspython python"
#+begin_src python
import argparse
import sys
...
try:
    ...
    print("错误: 缺少 'dnspython' 库。")
    print("请运行以下命令安装: pip install dnspython")
    sys.exit(1)
...
def query_dns(domain, record_type):
    ...
    try:
        ...
        print(f"[-] {record_type} 记录: 服务器返回空响应 (无记录)")
    except dns.resolver.NXDOMAIN:
        print(f"[-] 错误: 域名 '{domain}' 不存在 (NXDOMAIN)")
        return False # 域名不存在，停止后续查询
    except dns.resolver.NoNameservers:
        print(f"[-] {record_type} 记录: 所有名称服务器都无响应")
...
def main():
    parser = argparse.ArgumentParser(description="Python DNS 记录查询工具")
    parser.add_argument("domain", help="要查询的域名 (例如: google.com)", nargs='?', default=cf_domain)
    parser.add_argument("-t", "--type", help="指定的记录类型 (如 A, MX, NS, TXT)。如果不指定，则查询常用记录。", default=None)
    
    args = parser.parse_args()
    
    if args.type:
        # 如果指定了类型，只查询该类型（转为大写）
        query_dns(args.domain, args.type.upper())
    else:
        # 默认查询列表
