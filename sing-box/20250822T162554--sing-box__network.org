...
#+header: :var clash_mode=(if (zr-net-has-public-ipv6-addr-p) "ipv6" "ipv4")
  ...
  "route": {
    ...
    "rules": [
      ...
      },
      {
        "clash_mode": "ipv4",
        "outbound": "direct",
        "override_address": "<<find-ip(type=4)>>",
        "rule_set": "geoip-cloudflare"
      },
      {
        "clash_mode": "ipv6",
        "outbound": "direct",
        "override_address": "<<find-ip(type=6)>>",
        "rule_set": "geoip-cloudflare"
      },
      {
        "mode": "and",
...
(let* ((rules (zr-net-read-proxy-rules))
  ...
  (cl-mapc
   (lambda (p h)
     (when (string= "PROXY 127.0.0.1:10808" p)
       (push
...
(cl-letf
  ...
  (let ((result-dir (format "../cloudflarest/_results/%s-%d/" cdn type))
        (default "104.17.133.211")
        results)
    (mkdir result-dir t)
    (if-let* ((latest (car (last (directory-files result-dir t "^[^.]+\\.csv$")))))
        (extract-ip latest)
      default)))
#+end_src
