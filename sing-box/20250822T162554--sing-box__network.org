...
(let* ((final-dns "ld-neo")
  ...
  (json-serialize
   `((dns . ((final . ,final-dns)
             ...
             (rules . ,(cl-loop
                        with old-version = (version< sb-version-no "1.14.6") ; merge not support match_response in 1.14.0
                        for rule in (append pre-dns-client-rules dns-rules)
                        unless (and old-version
                                    (equal (alist-get 'action rule) "evaluate"))
                        collect (if old-version
                                    (assq-delete-all 'match_response rule)
                                  rule)
                        into rules
                        finally return (vconcat rules)))))
