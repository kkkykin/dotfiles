...
(let* ((box-secrets (with-temp-buffer
       ...
       (zr-org-tangle-default-dir
        (pcase system-type
          ...
          (_ "_tangle")))
       (sb-version (substring (car (process-lines "sing-box" "version")) 17))
...
(let* ((final-dns "ld-neo")
       ...
       (dns-rules `(((action . "predefined")
                    ...
                    ((domain_suffix . ,(concat "." mydomain))
                     (action . "predefined")
                     (answer . ,(cl-map 'vector (lambda (s) (format "*.%s. IN %s" mydomain s)) cf-ips)))
                    ((ip_accept_any . t)
                     (rule_set . "tailscale-rules")
