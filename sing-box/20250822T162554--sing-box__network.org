...
(let* ((box-secrets (with-temp-buffer
                      ...
                      (goto-char 1)
                      (json-parse-buffer)))
       (zr-org-tangle-default-dir
        (pcase system-type
          ('gnu/linux "/etc/sing-box/")
          (_ "_tangle")))
       (sb-version (process-lines "sing-box" "version"))
       (sb-version-no (nth 2 (split-string (car sb-version))))
       (client-tmp-dir (expand-file-name "tmp/" zr-org-tangle-default-dir))
       (tun-dir (expand-file-name "tun/" client-tmp-dir))
       (client-dir (expand-file-name "client/" zr-org-tangle-default-dir))
       (server-dir (expand-file-name "server/" zr-org-tangle-default-dir))
       (hosts-dir (expand-file-name "hosts/" zr-org-tangle-default-dir))
       ...
       (sublink-domain (or (gethash "sublink" clash-secrets)
                           "linuxdo.icmpmiao.cc"))
       (sub-dir (expand-file-name "sub/" zr-org-tangle-default-dir))
       ...
       (rule-sets-dir (expand-file-name "rule-sets/" zr-org-tangle-default-dir))
       (cf-domain "youxuan.cf.090227.xyz")
       (cf-dns (gethash "dns" box-secrets))
       (cf-dns-domain (gethash "domain" cf-dns))
       (cf-ips (cl-remove-duplicates (split-string (zr-org-babel-execute-named-src-block "dns-query") "\n" t)
                                     :test #'string=))
       (cf-4 "104.19.148.8")
       (cf-6 "2606:4700:e:5b:b68a:ec78:f652:5e89")
       server-route-users
       (client-proxy-users '("download"
                             "direct"
                             "browse"
                             "mitm"
                             "relay"
                             "tailscale"
                             "this-way"))
       (local-auth-port 10806)
       (local-auth-password "1")
       (local-anony-port 10807)
       (mydomain (auth-source-pick-first-password :host "mydomain" :user "main"))
       (hosts-array (gethash "hosts-array" box-secrets))
       (extra-dns-servers `(((accept_default_resolvers . :false)
                             (endpoint . "s[tailscale]")
                             (tag . "tailscale-dns")
                             (type . "tailscale"))))
       (dns4-tag "dns4-in")
       (wg-tag "wg-ep")
       pre-dns-client-rules
       pre-dns-server-rules
       (pre-hosts-tag "pre-hosts-dns")
                ...
                (when-let* ((type (alist-get 'type obd))
                            ((not (or (member type
                                      ...
                                      (member (alist-get 'type (alist-get 'transport obd))
                                              '("xhttp"
                                                "raw"))
                                      (member (alist-get 'fingerprint (alist-get 'utls (alist-get 'tls obd)))
                                              '("unsafe"))
  ...
  (dolist (ip cf-ips)
    (pcase (split-string ip)
      (`("AAAA" ,addr) (setq cf-6 addr))
      (`("A" ,addr) (setq cf-4 addr))))

  (let (hosts client-domains server-domains all-domains)
    (mapc
     (lambda (host)
       (let ((domain (gethash "domain" host)))
         (push (cons (intern domain) (gethash "ips" host)) hosts)
         (push domain (pcase (gethash "type" host)
    ...
    (push `((type . "hosts")
            ...
            (predefined . ,hosts))
          extra-dns-servers)
    (push `((domain . ,(vconcat server-domains all-domains))
            (server . ,pre-hosts-tag))
          pre-dns-server-rules)
    (push `((domain . ,(vconcat client-domains all-domains))
            (speculative . t)
            (server . ,pre-hosts-tag))
          pre-dns-client-rules))
  ...
  (let* ((coding-system-for-write 'utf-8))
    (cl-letf*
        (((symbol-function 'sb-process-dns-rules)
          (lambda (rules)
            (cl-loop
             with old-version = (version< sb-version-no "1.14")
             for rule across rules
             unless (and old-version
                         (member (alist-get 'action rule)
                                 '("evaluate"
                                   "respond")))
             collect
             (if old-version
                 (let ((rule (copy-tree rule)))
                   (when-let* ((sub-rules (alist-get 'rules rule)))
                     (setf (alist-get 'rules rule)
                           (sb-process-dns-rules sub-rules)))
                   (dolist (key '(match_response race speculative))
                     (setq rule (assq-delete-all key rule)))
                   rule)
               rule)
             into final
             finally return (vconcat final)))))
      (org-babel-tangle)))
    ...
    (zr-android-call-rish
     (string-join
      (list (format "sed -i 's,%s,%s,g' %s/*.json"
                    (expand-file-name zr-org-tangle-default-dir)
                    android-target-dir
                    android-dir)
  ...
  "route": {
    ...
    "rule_set": [
      ...
      },
      {
        ...
        "type": "remote",
        "url": "https://github.com/SagerNet/sing-geosite/raw/refs/heads/rule-set/geosite-adblockplus.srs"
      }
    ]
  }
}
...
***** dns
:PROPERTIES:
:CUSTOM_ID: 6e7026d8-9395-4dd9-9a60-b3b9a3edc5d1
:END:
#+begin_src json :tangle (zr-org-by-tangle-dir "500-dns.json") :mkdirp t
<<gen-server-dns()>>
#+end_src
...
*** client
:PROPERTIES:
:tangle-dir: (identity client-tmp-dir)
...
#+name: gen-client-main
#+begin_src elisp
(let* ((local-rulesets (directory-files rule-sets-dir t "\\.json$" t))
       (remote-rulesets '("gh/chocolate4u/Iran-sing-box-rules@rule-set/geoip-telegram.srs"
                          "gh/chocolate4u/Iran-sing-box-rules@rule-set/geoip-microsoft.srs"
                          "gh/chocolate4u/Iran-sing-box-rules@rule-set/geoip-cloudflare.srs"
                          "gh/chocolate4u/Iran-sing-box-rules@rule-set/geoip-google.srs"
                          "gh/SagerNet/sing-geoip@rule-set/geoip-cn.srs"
                          "gh/SagerNet/sing-geosite@rule-set/geosite-cn.srs"
                          "gh/SagerNet/sing-geosite@rule-set/geosite-geolocation-cn.srs"
                          "gh/SagerNet/sing-geosite@rule-set/geosite-geolocation-!cn.srs"
                          "gh/SagerNet/sing-geosite@rule-set/geosite-category-porn.srs"
                          "gh/SagerNet/sing-geosite@rule-set/geosite-category-ai-!cn.srs"
                          "gh/SagerNet/sing-geosite@rule-set/geosite-google.srs"
                          "gh/SagerNet/sing-geosite@rule-set/geosite-microsoft.srs"
                          "gh/SagerNet/sing-geosite@rule-set/geosite-telegram.srs"
                          "gh/SagerNet/sing-geosite@rule-set/geosite-talkatone.srs"
                          "gh/SagerNet/sing-geosite@rule-set/geosite-ebay.srs"
                          "gh/SagerNet/sing-geosite@rule-set/geosite-adblockplus.srs"))
       (jsdelivr "https://fastly.jsdelivr.net/")
       (cache-path (expand-file-name "cache.db" zr-org-tangle-default-dir))
       (main-route `((final . "s[direct]")
                     (rule_set
                      . ,(vconcat
                          (mapcar
                           (lambda (local)
                             `((path . ,(file-name-with-extension local "srs"))
                               (tag . ,(concat (file-name-base local) "-rules"))
                               (type . "local")))
                           local-rulesets)
                          (mapcar
                           (lambda (remote)
                             `((tag . ,(file-name-base remote))
                               (,(if (version< sb-version-no "1.14")
                                     'download_detour
                                   'http_client)
                                . "s[direct]")
                               (type . "remote")
                               (url . ,(concat jsdelivr remote))))
                           remote-rulesets)))))
       (exp-conf `((cache_file (,(if (version< sb-version-no "1.14")
                                     'store_rdrc
                                   'store_dns)
                                . :false)
                               (enabled . t)
                               (path . ,cache-path))))
       (main-conf `((log (level . "info")))))
  (if (version< sb-version-no "1.14")
      (push `(clash_api
              (access_control_allow_origin
               . ["https://board.zash.run.place"
                  "http://127.0.0.1"])
              (access_control_allow_private_network . t)
              (external_controller . "127.0.0.1:9090")
              (secret . ,(auth-source-pick-first-password
                          :host "clash" :user "secret")))
            exp-conf)
    (let ((default-client "s[direct]")
          (browse-client "s[browse]")
          (this-way-client "s[this-way]"))
      (push `(default_http_client . ,default-client)
            main-route)
      (push `(services
              . [((type . "api")
                  (secret . ,(auth-source-pick-first-password
                              :host "sing-box" :user "secret"))
                  (access_control_allow_origin . "http://127.0.0.1")
                  (listen . "127.0.0.1")
                  (listen_port . 9090)
                  (dashboard
                   . ((enabled . ,(if zr-sys-android-p :false t))
                      (http_client . ,this-way-client)
                      (path . ,(expand-file-name "dashboard"
                                                 zr-org-tangle-default-dir)))))])
            main-conf)
      (push `(http_clients . [((tag . ,default-client))
                              ((detour . ,browse-client)
                               (tag . ,browse-client))
                              ((detour . ,this-way-client)
                               (tag . ,this-way-client))])
            main-conf)))
  (push `(experimental . ,exp-conf) main-conf)
  (push `(route . ,main-route) main-conf)
  (json-serialize main-conf))
#+end_src
...
***** dns
:PROPERTIES:
:CUSTOM_ID: dede1c28-634e-4d83-ac7c-676a032f3b8c
:END:
#+begin_src json :tangle (zr-org-by-tangle-dir "_500-dns.json")
<<gen-client-dns()>>
#+end_src
...
(let* ((pre-rules `(((action . "sniff")
                    ...
                    ((mode . "and")
                     ...
                     (type . "logical"))
                    ((outbound . "s[direct]")
                     (rule_set . "tls-record-frag-rules")
                     (tls_record_fragment . t))
                    ((mode . "and")
                     (outbound . "s[browse]")
       ...
       (cf-group-rules `(((clash_mode . "cf-direct")
                         ...
                         ((clash_mode . "cf-v6")
                          ...
                          (rule_set . "geoip-cloudflare"))))
       (default-mode (cond
                      (zr-sys-android-p "cf-direct")
                      ((zr-net-has-public-ipv6-addr-p) "cf-v6")
                      (t "cf-v4"))))
...
SEP defaults to \"/\"."
    ...
    (let* ((mode-rules (apply #'vconcat (clash-expand-clash-mode-combinations
                                         mode-rules-group))))
      (json-serialize
       `((experimental (clash_api (default_mode . ,default-mode)))
         (route (rules . ,(vconcat pre-user-rules
                                   pre-rules
                                   mode-rules
                                   post-rules))))))))
#+end_src
...
(let* ((default '((localhost . ["127.0.0.1" "::1"])))
  ...
  (json-serialize
   `((dns . ((servers . [((tag . ,tag)
             ...
             (rules . ,(sb-process-dns-rules (vconcat rules))))))))
#+end_src
...
* bounds
:PROPERTIES:
:header-args:elisp: :var vps-secrets=(gethash "vps" box-secrets) wg-clients=(gethash "wg" box-secrets) ts-ep='((tag . "s[tailscale]") (type . "tailscale")) obds='(((tag . "s[direct]") (type . "direct"))) ibds='() ep='() conf='() dns-rules='() route-rules='()
:END:
*** client
#+name: gen-client-bounds
#+begin_src elisp
(let ((tailscale-state-dir (expand-file-name "tailscale_state" zr-org-tangle-default-dir))
      (download-tags '("s[this-way]" "s[direct]" "s[relay]"))
      (browse-tags '("s[this-way]" "s[direct]" "s[relay]"))
      relay-tags
      wg-rules-group
      wg-peers
      s-tags)
   ...
   (lambda (name attr)
     (let* ((user-name (gethash "user-name" attr))
       ...
       (when-let* ((port (gethash "nv-port" attr))
         ...
         (push tag vps-tags)
         (if (string-match-p "with_naive_outbound" (nth 3 sb-version))
             (push `((password . ,user-pass)
                     ...
                     (username . ,user-name))
                   obds)
           (let* ((local-port (gethash "nv-local-port" attr))
                  (listen (format "http://127.0.0.1:%d" local-port))
                  (proxy (format "quic://%s:%s@%s" user-name user-pass host))
                  (naive-dir (expand-file-name "naive/" zr-org-tangle-default-dir))
       ...
       (when vps-tags
         (dolist (user (alist-get name server-route-users nil nil #'equal))
           (let ((tag user))
             (push tag relay-tags)
             (push `((detour . ,s-tag)
                     (server . "127.0.0.1")
                     (server_port . ,local-auth-port)
                     (tag . ,tag)
                     (type . "socks")
                     (version . "5")
                     (username . ,user)
       ...
       (when-let* ((port (gethash "wg-port" attr))
                   ...
                   (proto (if (and (eq ip-type :ipv6)
                              ...
                              "ipv6"
                            "ip")))
         (push `( :address ,(gethash proto attr)
                  :port ,port
                  :persistent_keepalive_interval 30
                  :public_key ,pub
                  :allowed_ips ,(concat addr "/32"))
               wg-peers))))
   vps-secrets)
  ...
  (let* ((u-self-tag "u[self]")
         ...
         (u-dns "u[dns]")
         (s-dns (cons u-dns '("s[this-way]" "s[self]" "s[direct]")))
         pre-obds
         s-region
         region-obds)
    ...
    (push `(((type . "selector")
            ...
            ((type . "urltest")
             (tag . ,u-dns)
             (outbounds . ,(vconcat (seq-difference s-dns `(,u-dns "s[direct]"))))))
          pre-obds)
    (setq obds (append (apply #'append pre-obds) obds)))

  (push `((listen_port . ,local-auth-port)
          ...
          (users . ,(cl-map 'vector
                            (lambda (n)
                              `((username . ,n)
                                (password . ,local-auth-password)))
                            (append client-proxy-users
                                    (mapcan #'cdr server-route-users)))))
        ibds)
  (push `((listen_port . ,local-anony-port)
          (set_system_proxy . :false)
          (tag . "anony-mix")
          (type . "mixed"))
        ibds)
...
(let* ((attr (gethash name vps-secrets))
  ...
  (mapc
   (lambda (ip)
     (let* ((addr (gethash "addr" ip))
            (user (format "%s.%s" name (subst-char-in-string ?: ?_ addr))))
       (push user users)
       (push `((auth_user . ,user)
               (outbound . ,addr))
             route-rules)
       (push `((tag . ,addr)
               (type . "direct")
               ,(cons (intern (format "inet%d_bind_address" (gethash "type" ip)))
                      addr))
             obds)))
   (gethash "ips" attr))
  ...
  (maphash
   ...
   wg-clients)
  (when-let* ((port (gethash "wg-port" attr))
              (addr (concat (gethash "wg-addr" attr) "/24"))
  ...
  (when-let* ((warp (gethash "warp" attr))
              (addr (gethash "addr" warp))
              (priv-key (auth-source-pick-first-password :host "wgcf-warp" :user name))
              (warp-user (format "%s.warp" name))
              (ep-tag "wg-warp"))
    (push warp-user users)
    (push `((auth_user . ,warp-user)
            (outbound . ,ep-tag))
          route-rules)
    (push `((type . "wireguard")
            (tag . ,ep-tag)
            ...
            (private_key . ,priv-key)
            (mtu . 1280)
            (peers . [((address . "engage.cloudflareclient.com")
                       (port . 2408)
            ...
            (tls . ((enabled . t)
                    (reality . ((enabled . t)
                                (handshake . ((server . ,host)
                                              (detour . ,(alist-get 'tag (car obds)))
                                              (domain_resolver . ,pre-hosts-tag)
                                              (server_port . 443)))
  ...
  (when users
    ...
    (push `((listen . "127.0.0.1")
            ...
            (users . ,(cl-map 'vector
                              ...
                              users)))
          ibds)
    (push (cons name users) server-route-users)))
<<return-bounds>>
#+end_src
...
(when route-rules (push `(route . (rules ,(vconcat route-rules))) conf))
...
#+name: gen-client-dns
#+begin_src elisp
(let* ((final-dns "ld-neo")
       (cn-dns "ali-quic")
       (dns-servers `(((domain_resolver . ,cn-dns)
                       (path . "/dohgo")
       ...
       (dns-rules `(((action . "predefined")
                    ...
                    ((action . "reject")
                     (mode . "or")
                     (rules . [((rule_set . "geosite-adblockplus"))
                               ((rule_set . "block-rules"))])
                     (type . "logical"))
                    ((rule_set . "geosite-geolocation-cn")
                     (speculative . t)
  ...
  (json-serialize
   `((dns . ((final . ,final-dns)
             (strategy . ,(if (zr-net-has-public-ipv6-addr-p)
                              "prefer_ipv6"
                            "ipv4_only"))
             (servers . ,(vconcat extra-dns-servers dns-servers))
             (rules . ,(sb-process-dns-rules
                        (vconcat pre-dns-client-rules
                                 dns-rules)))))
     (route . ((default_domain_resolver . ,final-dns))))))
#+end_src

*** server
#+name: gen-server-dns
#+begin_src elisp
(let* ((final-dns "cf-dot")
       (tls-dns "cf-self-doh")
       (dns-servers `(((server . ,"1.1.1.1")
                       (tag . ,final-dns)
                       (type . "tls"))
                      ((server . ,cf-dns-domain)
                       (domain_resolver . ,final-dns)
                       (tag . ,tls-dns)
                       (path . ,(gethash "path" cf-dns))
                       (headers . ,(list (cons (intern (gethash "header-name" cf-dns))
                                               (gethash "header-value" cf-dns))))
                       (type . "https"))))
       (dot-rules `(((network . "tcp")
                     (inbound . ,dns4-tag)
  ...
  (json-serialize
   `((dns . ((final . ,final-dns)
             ...
             (servers . ,(vconcat dns-servers extra-dns-servers))
             (rules . ,(vconcat dot-rules pre-dns-server-rules))))
     (route . ((default_domain_resolver . ,final-dns)
               (rules
                . [((action . "sniff")
                    (sniffer . "dns"))
...
*** doh
:PROPERTIES:
:tangle-dir: (expand-file-name "doh/" zr-org-tangle-default-dir)
:END:

***** servers
:PROPERTIES:
:CUSTOM_ID: 3849cf51-5ab4-4229-8aed-495d5b9281c2
:END:
#+begin_src json :mkdirp t :tangle (zr-org-by-tangle-dir "servers.json")
<<gen-doh-servers()>>
#+end_src

#+name: gen-doh-servers
#+begin_src elisp
(let ((dns-servers '(((type . "https")
                      (tag . "cloudflare")
                      (server . "1.1.1.1"))
                     ((type . "https")
                      (tag . "quad9")
  ...
  (json-serialize (cl-remove-if
                   (lambda (s)
                     (not (member (alist-get 'type s) '("https" "hosts"))))
                   (vconcat dns-servers extra-dns-servers))))
#+end_src

***** rules
:PROPERTIES:
:CUSTOM_ID: e2de8371-7c74-470b-9423-0d9f66dfab10
:END:
#+begin_src json :mkdirp t :tangle (zr-org-by-tangle-dir "rules.json")
<<gen-doh-rules()>>
#+end_src

#+name: gen-doh-rules
#+begin_src elisp
(let* ((cf-api (org-file-contents "https://api.cloudflare.com/client/v4/ips" t))
       (cf-obj (and cf-api (json-parse-string cf-api)))
       (cf-cidr (if-let* (((not (null cf-obj)))
                          ((eq t (gethash "success" cf-obj)))
                          (result (gethash "result" cf-obj)))
                    (vconcat (gethash "ipv4_cidrs" result)
                             (gethash "ipv6_cidrs" result))
                  []))
       (dns-servers ["cloudflare" "quad9" "adguard"])
       (dns-rules `(((client_subnet . "114.114.114.0/24")
                     ,(cons (intern (format "header-%s" (gethash "header-name" cf-dns)))
                            (gethash "header-value" cf-dns))
                     (server . ,dns-servers)
                     (answer_rules
                      . [((ip_cidr . ,cf-cidr)
                          ...
                          (query . ,cf-domain)
                          (server . ,dns-servers))]))
                    ((client_subnet . "header:cf-connecting-ip")
                     (cf-ipcountry . "CN")
                     (server . ,dns-servers)
                     (answer_rules
                      . [((ip_cidr . ,cf-cidr)
                          ...
                          (query . ,cf-domain)
                          (server . ,dns-servers))])))))
  (json-serialize (vconcat pre-dns-client-rules dns-rules)))
#+end_src

*** script
#+name: dns-query
#+header: :results output
#+header: :var cf_domain=(identity cf-domain)
#+begin_src python
import argparse
import sys
import socket
...
def query_dns(domain, record_type):
    """
    查询指定域名的 DNS 记录，使用 socket.getaddrinfo 实现
    """
    try:
        if record_type == "A":
            # IPv4 地址查询
            results = socket.getaddrinfo(domain, None, family=socket.AF_INET)
            ...
            # IPv6 地址查询
            results = socket.getaddrinfo(domain, None, family=socket.AF_INET6)
            for result in results:
                addr = result[4][0]
                print(f"AAAA {addr}")
            return True
            ...
            print(f"[-] 不支持的记录类型: {record_type} (当前仅支持 A 和 AAAA)")
            return False

    except socket.gaierror as e:
        if e.errno == socket.EAI_NONAME:
            # print(f"[-] 错误: 域名 '{domain}' 不存在 (NXDOMAIN)")
            return False  # 域名不存在，停止后续查询
        elif e.errno == socket.EAI_NODATA or e.errno == socket.EAI_NONAME:
            # print(f"[-] {record_type} 记录: 无响应或无记录")
            return True
        else:
            # print(f"[-] 查询 {record_type} 时发生错误: {e}")
            return False
    except Exception as e:
        # print(f"[-] 查询 {record_type} 时发生未知错误: {e}")
        return False

def main():
    parser = argparse.ArgumentParser(description="Python DNS 记录查询工具 (基于 socket)")
    parser.add_argument("domain", help="要查询的域名 (例如: google.com)", nargs='?', default=cf_domain)
    parser.add_argument("-t", "--type", help="指定的记录类型 (目前仅支持 A, AAAA)", default=None)

    args = parser.parse_args()

    # 检查是否提供了域名
    if not args.domain:
        print("错误: 必须提供一个域名进行查询。")
        parser.print_help()
        sys.exit(1)

    if args.type:
        # 如果指定了类型，只查询该类型（转为大写）
        r_type = args.type.upper()
        if r_type not in ['A', 'AAAA']:
            print(f"[-] 不支持的记录类型: {r_type}。当前仅支持 A 和 AAAA。")
            sys.exit(1)
        query_dns(args.domain, r_type)
    else:
        # 默认查询列表
        default_types = ['A', 'AAAA']
        for r_type in default_types:
            # 如果查询过程中发现域名不存在（NXDOMAIN），则中断循环
            if not query_dns(args.domain, r_type):
                break
...
https://github.com/Chocolate4U/Iran-sing-box-rules/tree/rule-set
...
#+begin_src elisp
(let* ((rules (zr-sops-parse-json "../pac.json"))
...
*** tls-record-frag
:PROPERTIES:
:CUSTOM_ID: d94b5454-151d-4674-87f4-19d04f6282d2
:END:
#+begin_src json :tangle (zr-org-by-tangle-dir "tls-record-frag.json") :mkdirp t
<<gen-tls-record-frag-rule-set()>>
#+end_src

#+name: gen-tls-record-frag-rule-set
#+begin_src elisp
(let* ((default `(((domain_suffix
                    . [".weyolo.com"]))))
       (local-file "_tls-record-frag.eld")
       (local (org-file-contents local-file t t)))
  (json-serialize
   `((version . 3)
     (rules . ,(vconcat local default)))))
#+end_src
