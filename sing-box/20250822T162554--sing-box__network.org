...
* bounds
:PROPERTIES:
:header-args:elisp: :var vps-secrets=(gethash "vps" box-secrets) wg-clients=(gethash "wg" box-secrets) ts-ep='((tag . "s[tailscale]") (type . "tailscale")) obds='(((tag . "s[direct]") (type . "direct"))) ibds='() ep='() conf='() dns-rules='()
:END:
*** client
  ...
  (maphash
   (lambda (name attr)
     (let* ((user-name (gethash "user-name" attr))
       ...
       (push (format "warp[%s]" name) client-proxy-users)
