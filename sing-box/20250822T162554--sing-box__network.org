       ...
       (main-route `((final . "s[direct]")
                     (rule_set
                      . ,(vconcat
                          ...
                          (mapcar
                           (lambda (remote)
                             (let ((base `((tag . ,(file-name-base remote))
                                           (type . "remote")
                                           (url . ,(concat jsdelivr remote)))))
                               (if (version< sb-version-no "1.14")
                                   (cons '(download_detour . "s[direct]")
                                         base)
                                 base)))
                           remote-rulesets)))))
       (main-conf `((experimental
                     (cache_file (,(if (version< sb-version-no "1.14")
                                       'store_rdrc
                                     'store_dns)
                                  . :false)
                                 (enabled . t)
                    ...
                    (log (level . "info")))))
  (unless (version< sb-version-no "1.14")
    (let ((default-client "s[direct]"))
      (push `(default_http_client . ,default-client)
            main-route)
      (push `(http_clients . [((detour . "s[direct]")
                               (tag . ,default-client))])
            main-conf)))
