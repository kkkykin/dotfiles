...
# 获取脚本参数
name="$1"
shift # 移除第一个参数，剩余参数保留

# 定义路径
src="/storage/emulated/0/Android/media/com.termux/bin/${name}"
dst="/data/local/tmp/bin/${name}"
...
# 比较文件是否相同（内容一致且可执行）
if [ -f "$dst" ] && cmp -s "$src" "$dst" && [ -x "$dst" ]; then
  # 相同且可执行，直接运行
  exec "$dst" "$@"
else
  # 不同或不可执行，复制并赋予权限
  ...
  chmod 755 "$dst" || {
    echo "Error: Failed to chmod $dst" >&2
    exit 3
  }
  # 运行目标程序
  exec "$dst" "$@"
