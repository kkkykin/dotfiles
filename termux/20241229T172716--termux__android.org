...
(when (eq system-type 'android)
  (let ((termux-path (expand-file-name "usr/bin" zr-termux-root-directory))
    ...
    (dolist (bin (directory-files "_tangle/bin" t "\\.py$" t))
      (call-process "termux-fix-shebang" nil 0 nil bin))
...
:END:
#+header: :var EMACS_SOCKET_NAME=(if (boundp 'server-name) (expand-file-name server-name server-socket-dir) "")
#+header: :var emacsclient=(symbol-value 'emacsclient-program-name)
#+begin_src sh :tangle (if (and (eq 'android system-type) (display-graphic-p)) (zr-org-by-tangle-dir "emacs") "no")
set -euo pipefail
...
# 仅当 https_proxy 环境变量已设置且代理可用时才应用
if [[ -n "${https_proxy:-}" ]]; then
  ...
  elif [[ "$https_proxy" =~ ^https?://([^:]+):([0-9]+)$ ]]; then
    proxy_host="${BASH_REMATCH[1]}"
    proxy_port="${BASH_REMATCH[2]}"
  fi
  
  if [[ -n "$proxy_host" && -n "$proxy_port" ]]; then
    # 使用 nc 或 timeout 检测代理端口是否可达 (超时 2 秒)
    if timeout 2 bash -c ">/dev/tcp/$proxy_host/$proxy_port" 2>/dev/null; then
      echo "[fdroid] 使用代理: $https_proxy"
      export https_proxy
    else
      echo "[fdroid] 代理不可用，忽略: $https_proxy"
      unset https_proxy
    fi
  else
    echo "[fdroid] 无法解析代理地址，忽略: $https_proxy"
    unset https_proxy
  fi
fi
...
** audio-record
:PROPERTIES:
:CUSTOM_ID: 74b8d062-f203-42c2-9191-24f6b7224a4d
:END:
  ...
  case "$action" in
    ...
    *)
      ...
      if declare -f "$action" >/dev/null; then
        ...
        echo "Usage: $0 DAEMON_NAME {start|stop|stat|toggle|<custom>}"
        return 1
      fi
      ;;
  esac
...
** proxy
:PROPERTIES:
:CUSTOM_ID: c8fc3282-523d-4708-a145-864c4c16d1ff
:END:
#+begin_src sh :tangle (zr-org-by-tangle-dir "proxy")
PROXY_KEY="http_proxy"
DEFAULT_PROXY="127.0.0.1:10807"

# 获取当前代理（处理 null、空值和换行符）
get_proxy() {
  proxy=$(settings get global "$PROXY_KEY" 2>/dev/null)
...
enable_proxy() {
  ...
  if ! validate_proxy "$proxy"; then
    echo "错误: 代理格式无效 '$proxy'" >&2
    echo "正确格式: <IP地址>:<端口> (例如: 127.0.0.1:10807)" >&2
    return 1
  fi
  ...
  echo "正在设置代理: $proxy"
  if settings put global "$PROXY_KEY" "$proxy" 2>/dev/null; then
    echo "成功: 代理已启用 ($proxy)"
  else
    echo "错误: 设置失败，请检查 Shizuku 权限" >&2
    return 1
  fi
}
...
disable_proxy() {
  ...
  echo "正在禁用代理..."
  if settings put global "$PROXY_KEY" ":0" 2>/dev/null; then
    echo "成功: 代理已禁用"
  else
    echo "错误: 设置失败，请检查 Shizuku 权限" >&2
    return 1
...
show_help() {
  cat << EOF
代理管理脚本 - 通过 Shizuku (rish) 控制系统代理

用法: rish $0 <命令> [参数]

命令:
  enable [代理]   启用代理 (默认: $DEFAULT_PROXY)
  disable     禁用代理
  toggle      切换代理状态
  status      显示当前状态
  help      显示此帮助
...
示例:
  rish $0 enable
  rish $0 enable 192.168.1.100:8888
  rish $0 disable
  rish $0 toggle
  rish $0 192.168.1.10:8888
  rish $0 status
...
** private dns
:PROPERTIES:
:CUSTOM_ID: a67e6eca-4fa0-4d3a-8a7c-180aebf794a4
:END:
#+name: private-dns
#+begin_src sh :tangle (zr-org-by-tangle-dir "dot")
stat(){
  local mode
  mode=$(rish -c "settings get global private_dns_mode")
...
notify(){
  termux-notification \
    --ongoing -c "Private DNS Mode: ON" \
    -t "Android DOT" --group "$NOTIFICATION_GROUP" \
    -i "$NOTIFICATION_ID" --on-delete "$STOP_ACT"
}
...
stop(){
  rish -c "settings put global private_dns_mode off"
}
#+end_src
...
notify(){
  local system_proxy
  system_proxy="$($zr_cli proxy status)"
  termux-notification \
    --ongoing -c "sys: $system_proxy" -t "sing-box" \
    --group "$NOTIFICATION_GROUP" \
    -i "$NOTIFICATION_ID" \
    --action "$zr_cli proxy toggle && $zr_cli daemon sing-box notify" \
    --on-delete "$STOP_ACT"
}
...
start(){
  ...
  nohup "$zr_cli" runbin -c "sing-box run -c '$base_dir'/500-main.json > '$base_dir'/log 2>&1 &" >/dev/null 2>&1 &
}

stop(){
  rish -c 'settings put global http_proxy ":0" & pkill sing-box'
}
#+end_src
